...
const items = ref<MailAccount[]>([]);
const total = ref(0);
const importDialog = ref(false);
const importing = ref(false);
const showSecrets = ref(false);
const importText = ref("");
const importPlatform = ref("mailcom");
const filters = reactive({ q: "", platform: "", status: "", enabled: "" });
...
async function load() {
  ...
  try {
    const data = (
      await http.get("/admin/mail-accounts", {
...
async function importAccounts() {
  if (!importPlatform.value.trim()) {
    ElMessage.warning("请选择邮箱平台");
    return;
  }
  ...
  try {
    const result = (
      await http.post("/admin/mail-accounts/import", {
...
async function toggle(item: MailAccount) {
  ...
  try {
    await http.patch(`/admin/mail-accounts/${item.id}/status`, {
      enabled: next,
    });
...
async function remove(item: MailAccount) {
  await ElMessageBox.confirm(
  ...
  try {
    await http.delete(`/admin/mail-accounts/${item.id}`);
...
<template>
  ...
  <el-card class="page-card" shadow="never">
    ...
    <el-table v-loading="loading" :data="items" row-key="id">
      ...
      <el-table-column prop="mail" label="邮箱" min-width="240" />
      ...
      <el-table-column label="关联 Claude 账号" min-width="220">
        <template #default="{ row }">
  ...
  <el-dialog v-model="importDialog" title="批量导入邮箱账号" width="680">
    <el-alert
      title="先选择平台，每行粘贴一个“邮箱----密码”；重复邮箱会自动跳过。"
      type="info"
      :closable="false"
