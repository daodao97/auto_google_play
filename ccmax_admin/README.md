...
- 支持 3 分钟租约、幂等、使用次数均衡、单张库存不足自动创建 Slash 卡与结果上报的 Card 下发 API
...
Claude Free 账号租约默认 30 分钟；Google 账号和 Card 租约默认 3 分钟。分别通过 `ACCOUNT_DISPATCH_LEASE_MINUTES`、`GOOGLE_ACCOUNT_DISPATCH_LEASE_MINUTES`、`CARD_DISPATCH_LEASE_MINUTES` 配置。
...
下发后 Google 账号默认锁定 3 分钟。租约期间不会分配给其他请求；未上报时租约到期自动恢复可下发，上报后立即结束租约并执行结果状态：
...
响应中的 `cardPoolId` 用于升级关联、验证码查询和状态上报。每张 Card 下发后默认锁定 3 分钟，响应中的 `leaseExpiresAt` 为租约到期时间。租约期间不会分配给其他请求；未上报时租约到期自动恢复可下发。启用、未锁定且不在升级冷却期的卡会按使用次数均衡下发。
...
`status=used` 会立即解除租约、增加一次使用次数并让 Card 进入 5 小时冷却期；`status=unavailable` 会立即解除租约并将 Card 标记为不可用，管理员可在 Card Pool 中重新启用。`reason` 为可选审计说明。同一次下发的相同结果可安全重复提交。
